...
void magfreq_analysis_init(t_magfreq_analysis *x, short initialized)
{
    ...
    if(x->P <= 0)
        x->P = 1.0;
    if(!lpp_power_of_two(x->overlap))
        x->overlap = 2;
    if(!lpp_power_of_two(x->winfac))
        x->winfac = 2;
    x->N = x->D * x->overlap;
    x->Nw = x->N * x->winfac;
...
t_int *magfreq_analysis_perform(t_int *w)
{
    ...
    lpp_fold( input, Wanal, Nw, buffer, N, in );
    lpp_rdft( N, 1, buffer, bitshuffle, trigland );
    lpp_convert( buffer, channel, N2, lastphase_in, fundamental, factor_in );
